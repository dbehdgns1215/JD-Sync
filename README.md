...
그래서 사용자는 먼저 Notion에 `채용 일정` 데이터베이스를 하나 만들고, 그 데이터베이스를 Notion Calendar에 연결해야 합니다. 이후 자소설에서 즐겨찾기하면 JD-Sync가 그 데이터베이스에 새 줄을 만들고, Notion Calendar가 해당 줄의 `일정일`을 보여줍니다.

## Notion DB 칸 만들기

처음에는 아래 두 칸만 있으면 됩니다.
...
JD-Sync 설정의 `Notion DB 칸 이름`은 말 그대로 Notion DB에 만들어 둔 칸 이름입니다. 예를 들어 날짜 칸 이름이 `일정일`이면, JD-Sync가 새 공고 줄을 만들 때 그 칸에 공고 일정을 넣습니다. 공고 제목을 넣을 칸은 일반 텍스트 칸이 아니라 DB의 첫 번째 페이지 제목 칸이어야 합니다. Notion API에서 말하는 `rich_text`는 노션 화면의 텍스트 속성입니다. Notion Calendar 기본 DB를 그대로 쓴다면 제목 칸은 `액티비티`, 날짜 칸은 `날짜`로 입력하세요.
...
4. 데이터베이스의 첫 번째 페이지 제목 칸 이름을 `이름`으로 바꾸고, `일정일` 날짜 칸을 준비합니다.
