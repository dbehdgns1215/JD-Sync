...
JD-Sync 설정의 `Notion DB 칸 이름`은 말 그대로 Notion DB에 만들어 둔 칸 이름입니다. 예를 들어 날짜 칸 이름이 `일정일`이면, JD-Sync가 새 공고 줄을 만들 때 그 칸에 공고 일정을 넣습니다. Notion Calendar 기본 DB를 그대로 쓴다면 제목 칸은 `액티비티`, 날짜 칸은 `날짜`로 입력하세요.
...
4. 데이터베이스에 최소 `이름` title 칸과 `일정일` date 칸을 준비합니다.
